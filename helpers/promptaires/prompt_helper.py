...
class UserResponse:
    ...
    decided_response: Optional[str] = None
...
class Question:
    ...
    def is_multiple_choice(self) -> bool:
        return self.decision_choices is not None and len(self.decision_choices) > 0


def dict_to_question_prompt_factory(listed_dict: Dict[str, dict]) -> dict[str, Question]:
    """
    Takes in a dictionary of lists containing information for each question and returns a dictionary of questions.
    """
    question_dict = {}
    for question_key, question_info_list in listed_dict.items():
        print(question_key, question_info_list)
        question_dict[question_key] = Question(key=question_key,
                                              question=question_info_list['question'],
                                              question_type=question_info_list['question_type'],
                                              default_response=question_info_list['default_response'],
                                              response_type=question_info_list['response_type'],
                                              decision_choices=question_info_list.get('decision_choices', None),
                                              validation_func=question_info_list.get('validation_func', None),
                                              user_response=None)
...
class BasePrompt(TexiotyHelper):
    def __init__(self, txo: texoty.TEXOTY, txi: texity.TEXITY, prompt_name: str = "BasePrompt"):
        ...
        self.helper_commands['page'] = {
            ...
            'call_func': self.decisioning_page_change,
    ...
    def display_question(self):
        ...
        if self.current_question_index < len(self.question_keys):
            ...
            self.txo.priont_string(f"{question} [{self.question_prompt_dict[question_key].default_response}]")
...
def answer_to_response_factory(question: Question, answer: str) -> UserResponse:
    match question.response_type:
        ...
        case ResponseType.FLOAT:
            return UserResponse(question.key, question.response_type, float_response=float(answer))
        case ResponseType.INT:
            return UserResponse(question.key, question.response_type, int_response=int(answer))
        case ResponseType.BOOL:
            return UserResponse(question.key, question.response_type, bool_response=bool(answer))
        case ResponseType.LIST:
            return UserResponse(question.key, question.response_type, list_response=list(answer))
    return UserResponse(question.key, question.response_type, question.default_response)