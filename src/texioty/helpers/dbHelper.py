...
def create_table_statement_maker(table_name: str, column_names: list | dict) -> tuple[str, dict[Any, list[Any]]]:
    """
    ...
    if isinstance(column_names, dict):
        normalized_columns = list(column_names.keys())
    else:
        normalized_columns = list(column_names)

    if not normalized_columns:
        raise ValueError("No columns provided for table creation.")

    first_column = normalized_columns[0]
    add_new_table_info = {first_column: []}
    statement_maked = f'CREATE TABLE IF NOT EXISTS {table_name} ('
    statement_maked += f'{first_column} TEXT PRIMARY KEY,'
